...
const ABTestResults: React.FC<ABTestResultsProps> = ({
      ...
      >
        ...
        <div className="bg-background-light rounded-lg p-4 mb-4">
          <div className="flex items-center space-x-3 mb-3">
            ...
            <div>
              <p className="text-sm font-semibold text-text-primary">seu_perfil</p>
              <p className="text-xs text-text-muted">agora</p>
            </div>
          </div>
          
          {/* Mock image placeholder */}
          <div className="w-full h-48 bg-background-subtle rounded-lg flex items-center justify-center mb-3">
            <div className="text-text-muted text-sm">📸 Sua imagem aqui</div>
          </div>
          ...
          <div className="space-y-2">
            <p className="text-sm text-text-primary leading-relaxed">
              <span className="font-semibold">seu_perfil</span> {option.caption}
            </p>
  ...
  return (
    <div className="grid md:grid-cols-2 gap-6 animate-fade-in">
      <ContentCard option={optionA} label="Opção A" />
      <ContentCard option={optionB} label="Opção B" />
    </div>
  );
};
